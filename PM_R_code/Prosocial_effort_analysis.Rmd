...
d.tbt <- read.csv('data/lme4_PM_data_6_21.csv')                       %>%
  ...
  mutate(Recipient = factor(Recipient, 
                            levels = c('2', '1'), 
                            labels = c('Other', 'Self')))             %>% # recode as factor
...
# model_parameters(m.choice.glmer, standardize = "refit", exponentiate = T) %>% # if looking at results from fast version
  ...
  select(!c(df_error, Effects)) %>% 
  mutate(Parameter = str_replace(Parameter, pattern = "RecipientSelf", replacement = "Recipient (Self vs. Other)")) %>% 
...
main effects of recipient, effort, and reward (Figures 2a-b, S2 and Table S1).

## Analyse `r "<i>\u039A</i>"` parameters

```{r, warning=FALSE, message=FALSE, echo=FALSE}

# Wilcoxon signed-rank test on k parameterss for self vs other 
...
df.mi <- read_csv("data/Model_identifiability_mle_vba.csv", col_names = T)
...
plot.qcae.ae <- ggplot(d.wide, aes(x = other_ACCg_eff_RSA, y = QCAE_AE)) + 
  ...
  scale_x_continuous(n.breaks = 4, name = paste0(taulabel)) +
  ...
  annotate(geom='richtext', x = 0.08, y = 10,
           label = paste0("_r_","(", (length(!is.na(d.wide$QCAE_AE))-2),")=", format(round(o.accg.ae.corr[["estimate"]][["cor"]],digits=2), nsmall=2),", _p_", 
...
ACCg.roi.brain <- "../Figures_PM/ACCg_roi_zoom.jpg"
ACCg.roi.brain <- ggdraw() + draw_image(ACCg.roi.brain)

effort.rdm <- "../Figures_PM/Effort_task_RDM.jpg"
effort.rdm <- ggdraw() + draw_image(effort.rdm)

ACCg.force.brain <- "../Figures_PM/ACCg_force_zoom.jpg"
ACCg.force.brain <- ggdraw() + draw_image(ACCg.force.brain)

force.task <- "../Figures_PM/Force_task.jpg"
force.task <- ggdraw() + draw_image(force.task)

ACCg.roi.effort.rdm <- plot_grid(ACCg.roi.brain, NULL, effort.rdm, nrow = 3, rel_heights = c(3,0.5,1.5))

other.self.effort <- plot_grid(plot.Effort.ACCg, plot.qcae.ae, ncol = 2, rel_widths = c(0.9,1.4), align = "h")

ACCg.force.task <- plot_grid(ACCg.force.brain, NULL, force.task, nrow = 3, rel_heights = c(3,0.5,1.5))
...
self.other.RSA.brain <- "../Figures_PM/midbrain_SV_RSA.jpg"
...
SV.rdm <- "../Figures_PM/SV_RDM.jpg"
SV.rdm <- ggdraw() + draw_image(SV.rdm)

dACC.AI.SV.univ.brain <- "../Figures_PM/dACC_AI_SV_conj.jpg"
...
dACC.AI.SV.RSA.brain <- "../Figures_PM/dACC_AI_SV_RSA.jpg"
...
self.other.univ.brain <- "../Figures_PM/AI_SV_svc.jpg"
...
if (saveplots == 1) {tiff("figures/Figure_S3.tiff", units="in", res = resolution, width = corrplotW*2, height = plotH*1.25)}
...
kable(r.choice,
      align = c('l',rep('c',6)),
      caption = "Generalised linear mixed model for choice performance") %>% kable_styling()
...
kable(r.force,
      align = c('l',rep('c',3)),
      caption = "Linear mixed model for normalised force") %>% kable_styling()
