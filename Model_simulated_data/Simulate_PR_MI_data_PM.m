...
modPR = 7; % enter the model number to run PR on - numerical index in models variable **

% Load in schedule
% -------------------------------------------- %

load trial_order_PM_200_fmri.mat % specify trial order file here **
...
if any(contains(models, 'rew'))
    rewbounds = [0, 28]; % enter bounds on reward sensitivity values here **
else
    rewbounds = [];
end
...
kmin = min(kbounds);
kmax = max(kbounds);
rewmin = min(rewbounds);
rewmax = max(rewbounds);
...
for tr = toRun
    ...
    for r = 1:nRounds
        ...
        for m = modelsTR % loop over model number(s) specified above
            ...
            if type == 1 % if using a grid of values
                ...
                grid.k = [0:0.1:1.5]; % define grid values **
                grid.rew = [0:0.2:10]; % define grid values **
                grid.beta = [0:0.1:10]; % define grid values **
                
                for ip=1:length(params)
                    thisp=params{ip};
                    if contains(thisp, 'k') == 1
                        grid.all{ip} = grid.k;
                    elseif contains(thisp, 'rew') == 1
                        grid.all{ip} = grid.rew;
            ...
            elseif type == 2 % if using a distribution of values
                for sub = 1:nSubj
                    for ip = 1:length(params)
                        ...
                        elseif contains(thisp, 'rew') == 1
                            allCombs(sub,ip) = rand * rewmax; %  draw random value up to rew max (if min != 0 edit to account for) **
                            % can also define distribution other than normal e.g. betarnd() / gamrnd() 
                            while allCombs(sub,ip) > (rewmax) || allCombs(sub,ip) < (rewmin)
                                allCombs(sub,ip) = rand * rewmax;
                            end
                        elseif contains(thisp, 'beta') == 1
            ...
            for simS=1:nSubj
                ...
                %try
                [f,allout] = simfunc(Data(simS).effort, Data(simS).reward, Data(simS).agent, Data(simS).trueParam, modelID); %%%% VARIABLES TO FEED INTO THE MODEL IN ORDER TO SIMULATE CHOICES
                %catch
            ...
            if tr == 1 && mle_em == 2 % if using em for PR, fit all subjects
                ...
                [s, p] = em_PR(s, models(modelsTR), params, kbounds, betabounds);
                for simS=1:nSubj
                ...
                for param=1:nParam % plot correlations
                    ...
                    elseif contains(thisp, 'beta') == 1
                        ...
                        if mle_em == 1
                            ...
                            fittedKsBetas(1:fitSubj,param) = norm2beta_logsig(fittedParam(:,param));
                        end
                        plot(trueKsBetas(:,param),fittedKsBetas(:,param),'k.','MarkerSize',12);
                        all_corr(param,:) = corr(trueKsBetas(:,param),fittedKsBetas(:,param));
                    end
            ...
            elseif tr == 2
                ...
                if mle_em == 1
                    ...
                    [s, fits, fitMeasures] = mle_MI(s, models(modelsTR), 'bic', kbounds, rewbounds, betabounds, nrep);
                    all_s{r,m} = s;
                    all_fits{r,m} = fits;
    ...
    if type == 1
        name = ['workspaces/',toRunOptions{tr},'_',mleOptions{mle_em},'_k_',num2str(grid.k(1)),'_',num2str(grid.k(end)),'_b_',num2str(grid.beta(1)),'_',num2str(grid.beta(end)),'_n_',num2str(noise),'.mat'];
    elseif type == 2
