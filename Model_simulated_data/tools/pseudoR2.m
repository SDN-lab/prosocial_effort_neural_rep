...
nr_trials_raw = size(rootfile.beh{1,1}.agent,2); % get number of trials
...
while isnan(nllModel)
    try
        ...
        if isfield(rootfile.(fitops{EM+1}).fit.(modelID),'nll')
            for is = 1:n_subj
            nr_trials(is,1) = nr_trials_raw - sum(isnan(rootfile.beh{1,is}.choice));
            end
            nllModel = rootfile.(fitops{EM+1}).fit.(modelID).nll;
        end
    end
    try
