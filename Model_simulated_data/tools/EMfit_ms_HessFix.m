function [modout]= EMfit_ms_HessFix(rootfile, modelID, kbounds, betabounds)
...
%           [q,fval,ex,~,~,hessian] = fmincon(@all_real, q,[],[],[],[],lb,ub,[], options, rootfile.beh{is}.choice, rootfile.beh{is}.effort, rootfile.beh{is}.reward, rootfile.beh{is}.agent, modelID, 1);
%           [q,fval,ex,~,~,hessian] = fminunc(@all_real_minunc, q, fit.options, rootfile.beh{is}.choice, rootfile.beh{is}.effort, rootfile.beh{is}.reward, rootfile.beh{is}.agent, modelID, 1, kbounds, betabounds);
             
          inputfun = @(q)fit.objfunc(rootfile.beh{is}, q, fit, modelID, prior);
...
for is = 1:n_subj
   ...
   [nll_check,subfit]            = fit.objfunc(rootfile.beh{is}, m(:,is), fit, modelID, prior); 
   % group values
