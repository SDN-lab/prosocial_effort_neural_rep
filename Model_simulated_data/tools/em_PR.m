function [s, fitted] = em_PR(s,modelsTR, params, kbounds, betabounds)
% Fits effort models and does model comparison on simulated data for parameter
% recovery
% Patricia Lockwood, January 2020, Based on code by MK Wittmann, October 2018
% Applied to parameter recovery by Jo Cutler August 2020
%
...
for iexp = 1:numel(expids)
    ...
    for im = 1:numel(M.modid) % for the number of models
        ...
        while 1==dotry
            % try
            %          close all;
            s.(cur_exp).em = EMfit_ms_HessFix(s.(cur_exp),M.modid{im}, kbounds, betabounds);dotry=0;
            %         catch
            %            dotry=1; disp('caught');
            %         end
        end
