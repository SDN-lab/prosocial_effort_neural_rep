...
%     Data.chosen = chosen(~isnan(chosen))';
%     allout.choice = chosen(~isnan(chosen))';
%     Data.reward = reward(~isnan(reward))';
%     Data.effort = effort(~isnan(effort))';
%     Data.agent  = agent(~isnan(agent))';
...
%         ub = [1.5 10]; %upper bounds on parameters
%         
...
%         ub = [1.5 1.5 10]; %upper bounds on parameters
%         
%         %%% I.) first fit the model:
%         outtype=1;
...
%         ub = [1.5 1.5 10 10]; %upper bounds on parameters
%         
%         %%% I.) first fit the model:
%         outtype=1;
...
%         
%         lb = [0 0 0];   %lower bounds on parameters
%         ub = [1.5 10 10]; %upper bounds on parameters
%         %%% I.) first fit the model:
%         outtype=1;
%         Parameter=[.1 .1 .1 ];                                                                                                                            % the starting values of the free parameters
...
%     end;
    ...
    %     fit.options = optimoptions(@fmincon,'Display','off','TolX',.0001,'MaxIterations',1000);
    %     allout.modelID = modelID;
    %     inputfun = @(p)fit.objfunc(allout,Data,Parameter);
    %     [out.x, out.fval, exitflag] = fmincon(inputfun,Parameter,[],[],[],[],lb,ub,[],fit.options); %%%% GOES INTO ORIGINAL MODEL HERE FOR FITTING
    
    %%% II.) Get modeled schedule:
    outtype=2;
    Parameter=out.x;
    %     [~, modelout] = all_real(allout, Data, Parameter); % type 2
