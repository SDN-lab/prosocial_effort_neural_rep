function [s, fits, fitMeasures] = mle_MI(s, modelsTR, beston, kbounds, rewbounds, betabounds, nrep)
...
for iexp = 1:numel(expids)
    ...
    for im = 1:numel(M.modid) % for the number of models
        modelID=M.modid{im};
        for i=1:nrep
            s.(cur_exp).ml.(modelID) = fit_PM_model(s, modelID, kbounds, rewbounds, betabounds, i);
        end
    end
end
